...
Counts and metadata match the main analysis. `Subgroup` can list multiple labels per sample (comma-separated); we split and treat each label as its own subgroup list. The chunk loads everything once so each subgroup reuses the same counts and metadata without depending on repeated external annotation lookups.
...
# all configured hosts, which blocks subgroup rendering even when the rest of
# the analysis is healthy. Re-enable when a stable annotation source is
# available for automated builds.
