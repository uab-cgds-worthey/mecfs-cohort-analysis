...
all_samples_output_dir <- file.path("output", "all-samples-analysis")
dir.create(all_samples_output_dir, recursive = TRUE, showWarnings = FALSE)
```

# Overview

This notebook fits Affected vs Unaffected with `~ Batch + Affected` on all samples in the metadata (no removal of related individuals). The [subsetted analysis](subsetted-condition-analysis.html) applies the same model after excluding one member per multiplex family so independence assumptions are less strained for inference.

Outputs here go to `output/all-samples-analysis/` and support exploratory comparisons; publication-style inference follows the subsetted page.

# DGE Analysis Setup

Load libraries and `code/helpers.R` with `renv` active so package versions match the project lockfile.
...
Counts come from star-salmon; metadata is the project CSV in `data/`. `check_order()` aligns matrix columns with metadata rows by sample ID.
...
# BioMart calls are disabled here because the CI environment can fail across
# all configured hosts, which blocks the rest of the report from rendering.
# Re-enable when a stable annotation source is available for automated builds.
...
DESeq2 tests differential expression on counts with `~ Batch + Affected` so batch is a nuisance term and Affected carries the scientific contrast. Size factors, dispersions, and Wald statistics follow the standard pipeline (see subsetted write-up for interpretation).
...
`vst()` produces a variance-stable matrix for PCA and heatmaps; it does not replace the count-based model for p-values.
...
`removeBatchEffect()` on the VST matrix removes Batch for plotting while retaining Affected structure when the design includes both.

```{r, limma}
counts_vst <- assay(vsd)
write.csv(counts_vst, file = file.path(all_samples_output_dir, "counts_vst.csv"))
...
write.csv(counts_vst_limma, file = file.path(all_samples_output_dir, "counts_vst_limma.csv"))
...
Euclidean distance on batch-corrected VST; darker tiles mean more similar expression profiles.
...
On batch-corrected VST, PCs do not separate batches cleanly, which is what we want after `removeBatchEffect()`. The three male samples partially separate from most females on these PCs, so sex structure is visible in exploratory space. The subsetted analysis applies a different restriction: it keeps one sample per multiplex family (see that page for excluded IDs), which is the rule used for publication-style inference.
...
We define annotation colors and summarize the most variable genes to visualize major expression structure across the cohort.
...
This contrast estimates differential expression between affected and unaffected groups under the fitted model.
...
This heatmap focuses on statistically significant genes to show direction and consistency of expression changes across samples.
...
res_aff_vs_unaff_df_padj05_lfc1 <- res_aff_vs_unaff_df_padj05_lfc1[order(res_aff_vs_unaff_df_padj05_lfc1$padj), ]
...
The volcano plot combines effect size and adjusted significance to prioritize genes with strong and reliable differences.
...
write.csv(missing_gene_counts,
          file = "output/all-samples-analysis/missing_genes_interest_counts.csv")
...
Here we scrutinize predefined biologically relevant genes with per-gene count plots and summary heatmaps.
...
df <- as.data.frame(colData(vsd_limma)[, c("Affected", "Category")])
...
ragg::agg_png("output/all-samples-analysis/genes_of_interest_heatmap.png", width = 12, height = 10, units = "in", res = 1200)
...
This section isolates the RBC-focused candidate subset to inspect whether the same case-control patterns persist in a narrower panel.
...
ragg::agg_png("output/all-samples-analysis/slc4a1_genes_of_interest_heatmap.png", width = 14, height = 10, units = "in", res = 1200)
...
We dissect the MADD-related candidate set separately to test whether this pathway-specific signature is coherent across samples.
...
ragg::agg_png("output/all-samples-analysis/madd_genes_of_interest_heatmap.png", width = 14, height = 8, units = "in", res = 1200)
...
We assess the mitochondrial candidate set separately to evaluate whether energy metabolism genes show coordinated case-control shifts.
...
ragg::agg_png("output/all-samples-analysis/mito_genes_of_interest_heatmap.png", width = 14, height = 8, units = "in", res = 1200)
...
This section provides per-sample count views for significant DEGs to validate effect direction and identify outlier-driven signals.
...
This aggregate heatmap combines key gene sets into one view to compare global expression patterns across affected status and category.
...
ragg::agg_png(file_name, width = 16, height = 26, units = "in", res = 1200)
draw(
...
This focused heatmap summarizes the curated gene list used in the manuscript to support interpretation of biologically motivated targets.

```{r, heatmap-of-all-samples-genes-of-interest}
genes_of_interest <- read_csv("data/Manually_Curated_Genes_List_2024_12_05.csv")
genes_of_interest <- unique(genes_of_interest$Genes)
genes_of_interest <- genes_of_interest[!is.na(genes_of_interest)]

output_path <- "output/all-samples-analysis/"
file_name <- file.path(output_path, "Supplemental Figure 3. Genes of Interest Heatmap.png")
...
We save the workspace so every intermediate object can be revisited for reproducibility and downstream checks.
