...
rownames(sample_dist_matrix) <- paste(vsd_limma$Batch, vsd_limma$Family, sep = " | ")
colnames(sample_dist_matrix) <- paste(vsd_limma$RNA_Samples_id, vsd_limma$Family, sep = " | ")
colors <- colorRampPalette(rev(brewer.pal(9, "Blues")))(255)

pheatmap(sample_dist_matrix, clustering_distance_rows = sampleDists, clustering_distance_cols = sampleDists, col = colors)
```
...
pca_data <- plotPCA(vsd, intgroup = c("Batch", "Family", "Affected"), returnData = TRUE)
percent_var <- round(100 * attr(pca_data, "percentVar"))

ggplot(pca_data, aes(PC1, PC2, shape = factor(Batch), fill = factor(Affected), color = factor(Family))) +
  geom_point(size = 5) +
  geom_text(aes(label = name),
    data = subset(pca_data, PC2 < -20),
...
ggplot(pca_data, aes(PC1, PC2, shape = factor(Batch), color = factor(Affected))) +
  geom_point(size = 5) +
...
top_var_genes_100 <- head(order(-rowVars(assay(vsd_limma))), 100)
mat_100 <- assay(vsd_limma)[top_var_genes_100, ]
mat_100 <- mat_100 - rowMeans(mat_100)
df_100 <- as.data.frame(colData(vsd)[, c("Batch", "Family", "Affected")])
...
Below is a heatmap highlighting the expression of our genes of interest across batch, disease type, and affected status.
