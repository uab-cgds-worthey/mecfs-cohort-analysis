...
```{r, load libraries}
library(tidyverse) # Avaialble via CRAN
library(DESeq2) # Available via Bioconductor
library(RColorBrewer) # Avaialble via CRAN
...
```{r, padj<0.05 & heatmap of top 50 genes}
library(pheatmap)
topgenes_aff_vs_unaff_05 = head(rownames(res_aff_vs_unaff_05),50)
topgenes_aff_vs_unaff_05
topgenes_aff_vs_unaff_05 = assay(vsd_limma)[topgenes_aff_vs_unaff_05,] 
topgenes_aff_vs_unaff_05 = topgenes_aff_vs_unaff_05 - rowMeans(topgenes_aff_vs_unaff_05)
df = as.data.frame(colData(vsd_limma)[,c("Batch", "Family", "Affected")])
png("output/heatmap_res_aff_vs_unaff_05_top-50.png", width = 1024, height = 768)
pheatmap(topgenes_aff_vs_unaff_05, annotation_col=df)
dev.off()
```
...
```{r, adding gene name  and padj<0.05 }
...
head(res_aff_vs_unaff_df_genename_05)
summary(res_aff_vs_unaff_df_genename_05)
