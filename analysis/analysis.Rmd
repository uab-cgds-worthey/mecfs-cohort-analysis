...
ggplot(pcaData, aes(PC1, PC2, shape=factor(Batch), fill=factor(Affected),  color=factor(Family))) + geom_point(size=5) + 
  geom_text(aes(label = name), data=subset(pcaData, PC2 < -20),
...
maplot <- ggmaplot(res_aff_vs_unaff_df_genename, main = "Affected vs Unaffected MA Plot",
...
8 of the genes were not included using DESeq2's count filtering methods.
...
missing_indices <- match(missing_genes, subset_gene_info$Ensembl_ID)

# Retrieve corresponding gene_name values
corresponding_gene_names <- subset_gene_info$gene_name[missing_indices]
