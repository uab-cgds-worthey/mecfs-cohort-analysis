...
for (ensembl_id in filtered_by_interest$Ensembl_ID) {
  ...
  # Calculate the average counts by condition
  avg_counts_by_condition <- d %>%
    group_by(Affected) %>%
  ...
  # Loop through each row (patient) to determine regulation status
  regulation_status <- sapply(1:nrow(d), function(i) {
    patient_count <- d$count[i]
    condition <- d$Affected[i]
    avg_count <- avg_counts_by_condition$avg_count[avg_counts_by_condition$Affected == condition]
    
    # Determine regulation status with 25% threshold
    if (patient_count > 1.25 * avg_count) {
      return("Upregulated")
    } else if (patient_count < 0.75 * avg_count) {
...
for (ensembl_id in res_aff_vs_unaff_df_genename_1$Ensembl_ID) {
  ...
  regulation_status <- sapply(1:nrow(d), function(i) {
    patient_count <- d$count[i]
    condition <- d$Affected[i]
    avg_count <- avg_counts$avg_count[avg_counts$Affected == condition]
