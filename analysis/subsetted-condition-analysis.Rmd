...
```{r}
res_aff_vs_unaff_sub <- results(dds, contrast = c("Affected", "Yes", "No"))
...
pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 3.5)
```
...
topgenes_byensemblid_sub <- head(rownames(res_aff_vs_unaff_df_sub), 57)
topgenes_aff_vs_unaff_05_sub <- assay(vsd_limma)[topgenes_byensemblid_sub, ]
topgenes_aff_vs_unaff_05_sub <- topgenes_aff_vs_unaff_05_sub - rowMeans(topgenes_aff_vs_unaff_05_sub)
...
gostplot(res, capped = FALSE, interactive = FALSE)
```
...
```{r, enrichplot-disgenet}
top_genes_df <- res_aff_vs_unaff_df_sub[order(-abs(res_aff_vs_unaff_df_sub$log2FoldChange)), ]
