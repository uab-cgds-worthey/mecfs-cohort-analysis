...
# Import metadata
sample_metadata <- read_csv("data/MECFS_RNAseq_metadata_2024_03_14.csv")
...
ComplexHeatmap::pheatmap(mat, annotation_col = df, annotation_colors = ann_colors2, 
...
res_aff_vs_unaff_df_1_sub <- subset(res_aff_vs_unaff_df_sub, padj < 0.05)
...
ComplexHeatmap::pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 5, color = colorRampPalette(c("navy", "white", "firebrick3"))(20), angle_col = c("0"))
```
...
pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 5, color = colorRampPalette(c("navy", "white", "firebrick3"))(20), angle_col = 0)
```
...
pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 5)
```
...
pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub,
...
df_sub <- as.data.frame(colData(vsd_limma)[, c("Affected", "CombinedCategory")])
pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 1.5)
```
...
df_sub <- as.data.frame(colData(vsd_limma)[, c("Affected", "CombinedCategory")])
pheatmap(topgenes_aff_vs_unaff_05_sub, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 3.75)
```
...
df_sub <- as.data.frame(colData(vsd_limma)[, c("Affected", "SubCategory")])
pheatmap(mat3, annotation_col = df_sub, annotation_colors = ann_colors2, fontsize = 4)
```
...
# Plot results
barplot(ego_cc, showCategory = 10, title = "Overrepresented Cellular Components") + ggthemes::theme_clean()
```
...
# Plot results
barplot(ego_bp, showCategory = 10, title = "Overrepresented Biological Processes") + ggthemes::theme_clean()
```
...
```{r, kegg-plot}
kk2 <- gseKEGG(geneList     = gl_sorted,
...
dotplot(y, title = "Reactome Pathway Over-representation Analysis") 
```
...
for (ensembl_id in filtered_by_interest$Ensembl_ID) {
  ...
  ggplot_box <- ggboxplot(d, x = "Affected", y = "count", add = "jitter",
                          color = "Affected", palette = c("navy", "red"),
                          title = gene_name) +
    geom_text_repel(aes(label = rownames(d))) +
    scale_color_manual(name = "Affected Status",
                       labels = c("Unaffected", "Affected"),
                       values = c("navy", "red"))
