...
pca_data_all <- plotPCA(vsd_limma, intgroup = c("Category"), returnData = TRUE)
percent_var_all <- round(100 * attr(pca_data_all, "percentVar"))
...
df_100_all <- as.data.frame(colData(vsd_limma)[, c("Affected", "Category")])
...
df_m <- df <- colData(vsd_limma) %>% as.data.frame() %>% dplyr::select(Affected, Category)
