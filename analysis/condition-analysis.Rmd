...
colnames(sample_dist_matrix_all) <- paste(vsd_limma$RNA_Samples_id, vsd_limma$Affected, sep = " | ")
colors <- colorRampPalette(rev(brewer.pal(9, "Blues")))(255)
...
mat <- mat - rowMeans(mat)
df <- as.data.frame(colData(vsd_limma_female)[, c("Batch", "Affected", "Disease")])
...
df_all <- as.data.frame(colData(vsd_limma)[, c("Batch", "Affected", "Disease")])
...
df_100 <- as.data.frame(colData(vsd_limma_female)[, c("Batch", "Affected", "Disease")])
...
pheatmap(mat_100, annotation_col = df_100, annotation_colors = ann_colors, fontsize = 6)
```
...
df_100_all <- as.data.frame(colData(vsd_limma)[, c("Batch", "Affected", "Disease")])
...
pheatmap(mat_100_all, annotation_col = df_100_all, annotation_colors = ann_colors, fontsize = 6)
```
...
df <- as.data.frame(colData(vsd_limma_female)[, c("Batch", "Affected", "Disease")])
pheatmap(topgenes_aff_vs_unaff_05, annotation_col = df, annotation_colors = ann_colors, fontsize = 5)
```

```{r, top-100-genes-deseq-all, out.height="570px", out.width ="800px"}
topgenes_byensemblid_m <- head(rownames(res_aff_vs_unaff_df_m), 100)
topgenes_aff_vs_unaff_05_m <- assay(vsd_limma)[topgenes_byensemblid, ]
...
df_m <- as.data.frame(colData(vsd_limma)[, c("Batch", "Affected", "Disease")])
pheatmap(topgenes_aff_vs_unaff_05_m, annotation_col = df_m, annotation_colors = ann_colors, fontsize = 5)
```
...
significant_data <- ifelse(grepl("Up", significant_data$sig), "Up",
...
write.csv(significant_data, file = "output/res_aff_vs_unaff_significant_females_only.csv", row.names = FALSE)
```
