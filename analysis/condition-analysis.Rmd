...
pca_data <- plotPCA(vsd_limma, intgroup = c("Batch", "Affected", "Sex"), returnData = TRUE)
...
pca_data <- plotPCA(vsd_limma_female, intgroup = c("Batch", "Affected", "Ancestry"), returnData = TRUE)
percent_var <- round(100 * attr(pca_data, "percentVar"))
...
ggplot(pca_data, aes(PC1, PC2)) +
  ...
  scale_color_manual(values=c("blue","red"))+
  geom_text(aes(label = name),
    data = subset(pca_data, Ancestry == "Black" ),
    vjust = -1, hjust = 0.5, size = 2.5
  ) +
  xlab(paste0("PC1: ", percent_var[1], "% variance")) +
  ylab(paste0("PC2: ", percent_var[2], "% variance")) +
