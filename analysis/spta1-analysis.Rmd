...
pca_data_all <- plotPCA(vsd_limma, intgroup = c("Batch", "Variant"), 
                        returnData = TRUE)
...
pca_data_all <- plotPCA(vsd_limma, intgroup = c("OverallCategory"), 
                        returnData = TRUE)
percent_var_all <- round(100 * attr(pca_data_all, "percentVar"))
...
res_aff_vs_unaff_df <- process_and_save_results(
  res_aff_vs_unaff,
  "output/spta1/res_aff_vs_unaff.csv"
)
...
res_aff_vs_unaff_df_padj1_lfc1 <- subset(res_aff_vs_unaff_df_1, log2FoldChange >= 1 | log2FoldChange <= -1)
...
```{r, top-100-genes-deseq, out.height="570px", out.width ="800px"}
...
df <- as.data.frame(colData(vsd_limma)[, c("Variant", "OverallCategory")])
pheatmap(topgenes_aff_vs_unaff_05, annotation_col = df, annotation_colors = ann_colors3, fontsize = 5, angle_col = c("0"))
```

```{r, top-100-genes-deseq-subcategory, out.height="570px", out.width ="800px"}
topgenes_byensemblid <- head(rownames(res_aff_vs_unaff_df), 100)
topgenes_aff_vs_unaff_05 <- assay(vsd_limma)[topgenes_byensemblid, ]
topgenes_aff_vs_unaff_05 <- topgenes_aff_vs_unaff_05 - rowMeans(topgenes_aff_vs_unaff_05)
...
```{r, top-50-genes-deseq, out.height="570px", out.width ="800px"}
...
```{r, top-50-genes-deseq-subcategory, out.height="570px", out.width ="800px"}
topgenes_byensemblid <- head(rownames(res_aff_vs_unaff_df), 50)
topgenes_aff_vs_unaff_05 <- assay(vsd_limma)[topgenes_byensemblid, ]
topgenes_aff_vs_unaff_05 <- topgenes_aff_vs_unaff_05 - rowMeans(topgenes_aff_vs_unaff_05)
...
df <- df <- colData(vsd_limma) %>%
  as.data.frame() %>%
  dplyr::select(Affected, SubCategory, OverallCategory)
pheatmap(topgenes_aff_vs_unaff_05,
  annotation_col = df,
  annotation_colors = ann_colors3, fontsize = 5, angle_col =c("0")
)
```
...
```{r, significant-genes-deseq-padj1-lfc1, out.height="800px", out.width ="1400px"}
...
ComplexHeatmap::pheatmap(topgenes_aff_vs_unaff_05, color = colorRampPalette(rev(brewer.pal(n = 9, name =
  "RdYlBu")))(50), annotation_col = df, annotation_colors = ann_colors3, fontsize = 7, angle_col = c("0"), cellheight = 10, cellwidth = 24, border_color = "black", heatmap_legend_param = list(
        title = ""
    ))
```
...
write.csv(res_aff_vs_unaff_df_genename, file = "output/spta1/res_aff_vs_unaff_df_genename.csv")
```
...
ggsave(plot = ev, filename = "output/spta1-analysis/volcano-plot.png", device = "png", width = 14, dpi = 1200, units = "in")
```


```{r, significant-with-genename-padj-lfc}
# Process and save the data
process_and_save_results(res_aff_vs_unaff_df_genename, padj_threshold = 0.05, lfc_threshold = NULL, 
                         outpath = outpath_var, filename = "res_aff_vs_unaff_df_genename_05.csv")

process_and_save_results(res_aff_vs_unaff_df_genename, padj_threshold = 0.05, lfc_threshold = 1, 
                         outpath = outpath_var, filename = "res_aff_vs_unaff_df_genename_padj05_lfc1.csv")

process_and_save_results(res_aff_vs_unaff_df_genename, padj_threshold = 0.1, lfc_threshold = NULL, 
                         outpath = outpath_var, filename = "res_aff_vs_unaff_df_genename_padj1.csv")

process_and_save_results(res_aff_vs_unaff_df_genename, padj_threshold = 0.1, lfc_threshold = 1, 
...
maplot <- ggmaplot(res_aff_vs_unaff_df,
  main = "SPTA1 vs Unaffected MA Plot",
...
write.csv(significant_data, file = "output/spta1/res_aff_vs_unaff_significant_samples.csv", row.names = FALSE)
...
res_aff_vs_unaff_df <- res_aff_vs_unaff_df %>% dplyr::distinct()
geneList <- res_aff_vs_unaff_df$log2FoldChange
geneList2 <- res_aff_vs_unaff_df$padj
geneList3 <- res_aff_vs_unaff_df$padj
names(geneList3) <- rownames(res_aff_vs_unaff_df)
names(geneList2) <- rownames(res_aff_vs_unaff_df)
...
enrich_file_name <- "output/spta1/stringdb_combined_enrichment.csv"
write.csv(combined_enrichment, enrich_file_name, row.names = FALSE)
```


```{r, enrichr-analysis}
# # TODO - WEIRDLY NOT WORKING !!!!!
# listEnrichrSites()
...
#     file_name <- paste0("output/spta1/enrichr-dbs/", db_name, ".csv")
...
blood_genes <- c("AHSP", "HEMGN", "SLC4A1", "KEL", "SPTA1", "RHAG", 
                     "GYPE", "GYPA", "GYPB", "EPB42", "SPTB", "SPTBN5")

madd_genes <- c("ADM2", "TSHB", "PTH2", "ADORA2B", "GIP", "CGA", "CALCB", "IGHV3-16",
                "ADORA2A", "SYT6", "CYTH2", "VIPR2", "HTR6", "LHCGR", "GIPR", "CGB8",
                "DENND1A", "MADD", "DMXL2", "SMCR8", "TP53TG5", "IGHV3OR16-9")

mito_genes <- c("ETFB", "PGM1", "PCK2", "GLYCTK", "ACACB", "MLYCD", "MCCC1", 
               "CPT1A", "ELOVL4", "G6PD", "ACADVL", "ACADM", "HADHA", "MCCC2", 
               "CPT1C", "CPT1B", "ENO3", "SURF1", "CUBN", "MMACHC", "LMBRD1", 
               "MINPP1", "MMAA", "TACO1")


combined_genes <- c(genes_to_filter, madd_genes, mito_genes)
...
write.csv(filtered_without_genes, "output/spta1/other_genes_of_interest.csv", row.names = FALSE)

# Filtered data with only specific genes (genes_to_filter)
write.csv(filtered_with_only_genes, "output/spta1/slc4a1_genes_of_interest.csv", row.names = FALSE)

# Filtered data with only specific MADD genes
write.csv(filtered_with_only_madd_genes, "output/spta1/madd_genes_of_interest.csv", row.names = FALSE)

# Filtered data with only specific MADD genes
write.csv(filtered_with_only_mito_genes, "output/spta1/mito_genes_of_interest.csv", row.names = FALSE)
...
for (ensembl_id in filtered_without_genes$Ensembl_ID) {
  ...
  ggsave(
    ...
    path = "output/spta1/batch-correction-limma/plot-counts/genes-of-interest",
...
file_name <- "output/spta1/genes_of_interest_plot_counts_signals.csv"
...
png("output/spta1/genes_of_interest_heatmap.png", width = 12, height = 10, units = "in", res = 1200)
...
for (ensembl_id in filtered_with_only_genes$Ensembl_ID) {
  ...
  ggsave(
    filename = paste0(gene_name, "-plot-counts.png"),
    path = "output/spta1/batch-correction-limma/plot-counts/slc4a1-genes-of-interest",
    plot = gene_plot, dpi = 450
  )
}
...
file_name <- "output/spta1/slc4a1_genes_of_interest_plot_counts_signals.csv"
write.csv(gene_results_df, file_name, row.names = FALSE)
```
...
png("output/spta1/slc4a1_genes_of_interest_heatmap.png", width = 14, height = 10, units = "in", res = 1200)
...
for (ensembl_id in filtered_with_only_madd_genes$Ensembl_ID) {
  ...
  ggsave(
    filename = paste0(gene_name, "-plot-counts.png"),
    path = "output/spta1/batch-correction-limma/plot-counts/madd-genes-of-interest",
    plot = gene_plot, dpi = 450
  )
}
...
png("output/spta1/madd_genes_of_interest_heatmap.png", width = 14, height = 8, units = "in", res = 1200)
...
for (ensembl_id in filtered_with_only_mito_genes$Ensembl_ID) {
  ...
  ggsave(
    filename = paste0(gene_name, "-plot-counts.png"),
    path = "output/spta1/batch-correction-limma/plot-counts/mito-genes-of-interest",
    plot = gene_plot, dpi = 450
  )
}
...
file_name <- "output/spta1/mito_genes_of_interest_plot_counts_signals.csv"
write.csv(gene_results_df, file_name, row.names = FALSE)
```
...
png("output/spta1/mito_genes_of_interest_heatmap.png", width = 14, height = 8, units = "in", res = 1200)
...
for (ensembl_id in res_aff_vs_unaff_df_genename_padj05_lfc1$Ensembl_ID) {
  ...
  ggsave(
    filename = paste0(gene_name, "-plot-counts.png"),
    path = "output/spta1/batch-correction-limma/plot-counts/padj-05",
...
file_name <- "output/spta1/significant_plot_counts_signals.csv"
...
```{r, save-data}
save.image(file = "output/spta1/spta1-condition-analysis.RData")
```
